...
Адрес (необязательно): `address: "Арама 72"` — на сайте у места появятся адрес и кнопка 🗺 «Открыть на Яндекс Картах» (поиск этого адреса в Ереване); без адреса кнопка ищет по названию, поэтому сетям магазинов адрес не нужен. Адреса баров в `venues` (см. Beer Academy) главнее адреса места.
