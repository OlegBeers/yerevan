...
def _location(name: str, addresses: list[str], map_url: str | None = None) -> dict:
    ...
    return {"address": ", ".join(addresses) or None, "map_url": map_url or f"{YANDEX_MAPS}?{query}"}
...
def _place(place: Place, state: State, now: datetime) -> dict:
    ...
    return {
        ...
        **_location(place.name, place.addresses, place.map_url),
    }
